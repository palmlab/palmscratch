...
(function(ext) {
	...
	ext.resetAll = function(){
		// Naif: TODO: check compatibility of "0xff, 0x55, 2, 0, 4"
	...
	ext.runLed = function(on_off){
		//runPackage(6,short2array(on_off));
		if(on_off == "On")
			runPackage(6,1);
		else
			runPackage(6,0);
	...
	ext.runServoLook = function(angle, port) {
		if(typeof port=="string"){
			port = ports[port];
		}
	...
	ext.getButtonOnBoard = function(nextID,status){
		...
		getPackage(nextID,deviceId ,status);
	}
	...
	function sendPackage(argList, type){
		// Naif: TODO: check compatibility of "0xff, 0x55"
    ...
    function deviceOpened(dev) {
        ...
        device.set_receive_handler('palmbot',processData);
    };
    ...
    ext._getStatus = function() {
        if(!device) return {status: 1, msg: 'palmBot disconnected'};
        if(watchdog) return {status: 1, msg: 'Probing for palmBot'};
        return {status: 2, msg: 'palmBot connected'};
    }
    var descriptor = {};
	ScratchExtensions.register('palmBot', descriptor, ext, {type: 'serial'});
